...
export function ProductCard({ product, returnTo }: { product: Product; returnTo?: string }) {
  ...
  function add() {
    ...
    if (result.ok) showToast(t("product.added", { count: quantity, name: product.name }), "success");
    else showToast(t("product.missing", { amount: formatMoneyFromSek(result.missing, currency, locale) }), "error");
  }

  return (
    <article className="product-card">
      <Link href={productHref} className="focus-ring block aspect-[4/3] overflow-hidden bg-[#1a1a17]" aria-label={`${t("product.open")}: ${product.name}`}><ProductMedia product={product} compact /></Link>
      <div className="p-4">
        ...
        <Link href={productHref} className="mt-2 block"><h2 className="line-clamp-2 min-h-[2.8rem] font-display text-[1.35rem] leading-[1.05]">{product.name}</h2></Link>
