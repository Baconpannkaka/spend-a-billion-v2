    ...
    <>
      ...
      <section className="bg-[var(--paper)] py-7 text-[var(--ink)]"><div className="ad-shell"><AdPageLayout>
        <div className="filter-panel">
          <label className="relative block lg:col-span-2"><span className="sr-only">Sök produkter</span><Search className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-black/35" /><input value={filters.search} onChange={(event) => updateFilters({ search: event.target.value, page: 1 })} placeholder={mode === "luxury" ? t("shop.searchLuxury") : t("shop.searchEveryday")} className="field pl-10" /></label>
